...
const AchievementTable: React.FC<AchievementTableProps> = ({ records, onDeleteAll, onDeleteOne, onPreviewImage }) => {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `تقرير_التميز_المدرسي_${new Date().toLocaleDateString('ar-SA').replace(/\//g, '-')}.csv`);
  ...
  const completedCount = records.filter(r => r.status === 'تم الإنجاز').length;
  const incompleteCount = records.filter(r => r.status === 'لم يتم الإنجاز').length;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto mt-12 bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-gray-800 print:hidden overflow-visible">
      ...
      <div className="text-center mb-8">
        <h2 className="text-xl font-bold inline-block border-black pb-1">جدول إنجاز مقدم خدمات دعم التميز المدرسي</h2>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        ...
        <div className="flex flex-wrap gap-3 justify-center md:justify-start">
          ...
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6M8 13h8M8 17h8M10 9H8"/></svg>
            تصدير إلى Excel
          </button>
          <button 
            onClick={onDeleteAll} 
        ...
        <div className="flex gap-4 items-center">
          <div className="flex flex-col items-center bg-green-50 border border-green-200 px-4 py-1 rounded-xl">
            <span className="text-[10px] font-black text-green-600 uppercase">المنجزة</span>
            <span className="text-xl font-black text-green-700 leading-tight">{completedCount}</span>
          </div>
          <div className="flex flex-col items-center bg-red-50 border border-red-200 px-4 py-1 rounded-xl">
            <span className="text-[10px] font-black text-red-600 uppercase">غير المنجزة</span>
            <span className="text-xl font-black text-red-700 leading-tight">{incompleteCount}</span>
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-right border-collapse table-auto border border-gray-300">
          <thead>
            <tr className="bg-gray-100 text-[11px] font-black uppercase text-gray-700 border-b border-gray-300">
              ...
              <th className="px-2 py-3 border-l border-gray-300">حالة الإنجاز</th>
          ...
          <tbody>
            {records.length === 0 ? (
              <tr><td colSpan={11} className="px-4 py-16 text-center text-gray-400 italic font-bold">لا توجد بيانات مسجلة في التقرير حالياً</td></tr>
            ) : (
              records.map((rec, index) => (
                <React.Fragment key={rec.id}>
                  {/* Main Info Row */}
                  <tr className="bg-white border-b border-gray-200 text-[12px] transition-colors hover:bg-gray-50/50">
                    <td className="text-black px-2 py-4 border-l border-gray-200 font-black text-center">{index + 1}</td>
                    <td className="text-black px-2 py-4 border-l border-gray-200">{rec.semester}</td>
                    <td className="text-black px-2 py-4 border-l border-gray-200 font-black text-blue-700">{rec.team}</td>
                    <td className="text-black px-2 py-4 border-l border-gray-200 font-bold">{rec.supervisor}</td>
                    <td className="text-black px-2 py-4 border-l border-gray-200">{rec.day}</td>
                    <td className="text-black px-2 py-4 border-l border-gray-200 whitespace-nowrap font-medium">{rec.date}</td>
                    <td className="text-black px-2 py-4 border-l border-gray-200">{rec.school}</td>
                    <td className="text-black px-2 py-4 border-l border-gray-200">{rec.stage}</td>
                    <td className="text-black px-2 py-4 border-l border-gray-200 text-center">
                      <div className="flex items-center justify-center gap-2">
                         <span className={`inline-block px-3 py-1 rounded-full font-black text-[9px] ${
                        ...
                        <div className={`w-3 h-3 rounded-full shadow-inner ${rec.status === 'تم الإنجاز' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      </div>
                    </td>
                    <td className="px-2 py-4 border-l border-gray-200">
                      <div className="flex gap-1 flex-wrap justify-center">
                        ...
                        {rec.witness.length === 0 && <span className="text-gray-400 text-[10px]">لا يوجد</span>}
                      </div>
                    </td>
                      ...
                      <table className="w-full text-right bg-gray-50/50">
                        <thead>
                          <tr className="bg-gray-100/80 text-[10px] font-black text-gray-700 border-y border-gray-200">
                            <th className="px-4 py-2 border-l border-gray-200 w-1/4">المجال</th>
                            <th className="px-4 py-2 border-l border-gray-200 w-1/2">مؤشر الأداء</th>
                            <th className="px-4 py-2">الإجراءات</th>
                          </tr>
                        </thead>
                        <tbody>
                          {rec.details.map((detail, dIdx) => (
                            <tr key={dIdx} className="border-b border-gray-100 last:border-0">
